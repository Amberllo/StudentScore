...
public class AddScoreWindow extends JFrame{
	...
	private String sid;
	//
	public final static int FROM_MAIN_WINDOW = 1;//从管理员主界面跳转过来
	public final static int FROM_STUDENT_SCORE_WINDOW = 2;//从学生成绩界面跳转过来
	//
	...
	public AddScoreWindow(String sid){
		...
		initUI();
		initEvent();
		editSid.setText(sid);//从学生成绩界面跳转过来的话，把传过来的学号显示，并设置为不可编辑。
		...
		btnAdd.addActionListener(new ActionListener() {
			...
			public void actionPerformed(ActionEvent arg0) {
				...
				if(new ScoreDao().addSocre(new ScoreBean(sid,course,score,time))){
					...
					JOptionPane.showMessageDialog(null, "添加失败，系统出现问题!");
				}
			}
		});
